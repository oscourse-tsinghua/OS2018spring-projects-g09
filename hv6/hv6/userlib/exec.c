...
int exec_in_mem(void* data)
{
    ...
    for (i = 0; i < phnum; ++i, ++phdr) {
        ...
        assert(end_memva < KERNSTART, "must be lower than kernel");

        /* assume there's no overlap between sections */
        map_pages(rounddown(va, PAGE_SIZE), end_memva - rounddown(va, PAGE_SIZE), PTE_P | PTE_W | PTE_R | PTE_X | PTE_U);
        memcpy((void*)va, data+phdr->p_offset, phdr->p_filesz);
    }
    ...
    map_pages(USTACK_TOP - USTACK_SIZE, USTACK_SIZE, PTE_P | PTE_W | PTE_R | PTE_U);
...
static int clear_pt(pn_t pt)
{
    ...
    for (i = 0; i < PTRS_PER_PT; ++i) {
        ...
        pte_t* ptes = get_page(pt);
        entry = ptes[i];
        if (!(entry & PTE_P))
            continue;
        if (KERNSTART <= PTE_ADDR(entry) && PTE_ADDR(entry) < KERNEND) {
        	remain_count ++;
        	continue;
        }
...
static int clear_pdpt(pn_t pdpt)
{
    size_t i;
    int r;
    int remain_count = 0;
